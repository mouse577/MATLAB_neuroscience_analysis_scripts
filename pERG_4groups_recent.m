...
n=40 %enter total number of files to be included in analysis
M=cell(n,4)
...
%update numbers inside curly brackets
G3all=horzcat(M{21},M{22},M{23},M{24},M{25},M{26},M{27},M{28},M{29},M{30})

G3avg=mean(G3all,2)
[G3avgmin,idx]=min(G3avg(250:460),[],1)
G3minidx=idx
...
%update numbers inside curly brackets
G4all=horzcat(M{31},M{32},M{33},M{34},M{35},M{36},M{37},M{38},M{39},M{40})

G4avg=mean(G4all,2)
[G4avgmin,idx]=min(G4avg(250:494),[],1)
G4minidx=idx
...
G4finalresultscellavg={G4avgcell,G4avgtimecell,G4avgmax,G4idx,G4avgmin,G4minidx,G4ampavg,G4stderr}
...
plot(Tfold,G3stdplot(:,2),'bo','MarkerSize',10,'linewidth',2,'MarkerIndices',(G3minidx+250))
plot(Tfold,G4stdplot(:,2),'Color','g','linewidth',2)
plot(Tfold,G4stdplot(:,2),'g*','MarkerSize',10,'linewidth',2,'MarkerIndices',G4idx)
plot(Tfold,G4stdplot(:,2),'go','MarkerSize',10,'linewidth',2,'MarkerIndices',(G4minidx+250))
